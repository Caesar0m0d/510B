...
int n,m,l=-1,l2,l3,l4,rct_x=1,rct_y=1,xx=1;//make n+2,m+2 array
int x[2500],y[2500];
char turn=0;
...
{turn = maz[l3][l4];rct_x=l3;rct_y=l4;}
		else if((x[xx-1] != rct_x + 1) && (maz[rct_x + 1][rct_y] != 0) && ((maz[rct_x + 1][rct_y] == turn) || (maz[rct_x + 1][rct_y] == turn+32)))
{x[xx]=rct_x+1;y[xx]=rct_y; xx++;	
		if(maz[rct_x + 1][rct_y] == turn+32)printf("Yes\n");break;
 maz[rct_x][rct_y]+=32; rct_x=rct_x+1;rct_y=rct_y;
}
//pause
		else if((x[xx-1] != rct_x - 1) && (maz[rct_x - 1][rct_y] != 0) && ((maz[rct_x - 1][rct_y] == turn) || (maz[rct_x - 1][rct_y] == turn+32)))
{x[xx]=rct_x-1;y[xx]=rct_y; xx++;
...
{x[xx]=rct_x;y[xx]=rct_y+1; xx++;
		if(maz[rct_x][rct_y + 1] == turn+32)printf("Yes\n");break;
 maz[rct_x][rct_y]+=32; rct_x=rct_x;rct_y=rct_y+1;
}
//pause
		else if((y[xx-1] != rct_y - 1) && (maz[rct_x][rct_y - 1] != 0) && ((maz[rct_x][rct_y - 1] == turn) || (maz[rct_x][rct_y - 1] == turn+32)))
{x[xx]=rct_x;y[xx]=rct_y-1; xx++; 
